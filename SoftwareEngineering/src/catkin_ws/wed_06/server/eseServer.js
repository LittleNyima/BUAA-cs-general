...
var fileName = '/home/robot/catkin_ws/input.txt';

var script = 'screen_switch.txt';
...
// });
fs.writeFile(script, 'start',  function(err) {
   if (err) {
...
var worker1 = child_process.spawn('bash',['screen.sh']);
...
application.post('/grab', urlJson, function(req, res){
   fs.writeFile(script, 'stop',  function(err) {
      if (err) {
          return console.error(err);
      }
      console.log("脚本stop！");
